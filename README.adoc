...
For hash literals two styles are considered acceptable in the ruby community.
The first is to include padding inside the braces and is slightly more readable (and arguably more popular in the Ruby community in general).
The second is to omit any spaces and has the advantage of adding visual difference between block and hash literals.
Whichever one you pick - apply it consistently.
Agworld uses the former, with 1 space padding the braces of a hash literal.
...
Agworld uses the former; leading `.` when chaining methods.
...
=== Private Method Name Prefix [[private-method-prefix]]

Prefix private method names with an underscore character (`_`). Don't prefix protected method names.
...
=== Only mutate owned objects [[only-owned-mutations]]
